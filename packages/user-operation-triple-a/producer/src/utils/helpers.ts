...
import { PayCycle } from '@common/constants/paycycle';
...
import { BiWeeklyDisburseLog } from '@entities/BiWeeklyDisburseLog.entity';
import {
  add,
  addDays,
  addMonths,
...
export function calculateGrossDailyPay(pay_period_salary: number, pay_cycle: string) {
  if (pay_cycle === PayCycle.WEEKLY) {
    return pay_period_salary / 5;
  } else if (pay_cycle === PayCycle.BI_WEEKLY) {
    return pay_period_salary / 10;
  } else if (pay_cycle === PayCycle.MONTHLY) {
    return pay_period_salary / getDaysInMonth(new Date());
  } else if (
    pay_cycle === PayCycle.BI_MONTHLY ||
    pay_cycle === PayCycle.SEMI_MONTHLY
  ) {
    /**
     *
     * if day <=15 , divide by 15 else
     * divide by (daysInMonth - 15)
     *
     */
    ...
    return pay_period_salary / (getDaysInMonth(new Date()) - 15);
  }
}
export async function determinePayCycleDateRange(
	payCycle: string,
	employer_id: string,
) {
	let startDate, endDate;

	if (payCycle === PayCycle.DAILY) {
		startDate = new Date();
		endDate = addDays(new Date(), 1);
	} else if (payCycle === PayCycle.WEEKLY) {
		startDate = addDays(startOfWeek(new Date()), 1);
		endDate = subDays(endOfWeek(new Date()), 1);
	} else if (payCycle === PayCycle.BI_WEEKLY) {
		/**
		...
		const response = await getConnection()
			.getRepository(BiWeeklyDisburseLog)
			.findOne({
				where: {
		...
		endDate = addDays(new Date(response.start_date), 15);
	} else if (payCycle === PayCycle.MONTHLY) {
		startDate = startOfMonth(new Date());
		endDate = endOfMonth(new Date());
	} else if (payCycle === PayCycle.BI_MONTHLY) {
		if (getDate(new Date()) < 16) {
			startDate = startOfMonth(new Date());
			endDate = addDays(startOfMonth(new Date()), 14);
		} else {
			endDate = addDays(startOfMonth(new Date()), 15);
			endDate = endOfMonth(new Date());
		}
	} else if (payCycle === PayCycle.SEMI_MONTHLY) {
		if (getDate(new Date()) < 16) {
			startDate = startOfMonth(new Date());
			endDate = addDays(startOfMonth(new Date()), 14);
		} else {
			startDate = addDays(startOfMonth(new Date()), 15);
			endDate = endOfMonth(new Date());
		}
	}
